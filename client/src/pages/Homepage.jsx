    ...
    <div className="h-screen overflow-y-scroll text-white font-sans bg-[url('/bg-img.png')] bg-cover bg-center bg-no-repeat">
      ...
      <nav className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 ">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-semibold tracking-tight">Stackyn AI-Builder</span>
        </div>
        <div className="flex items-center gap-4 text-sm font-medium text-zinc-300">
          <span>{user?.name}</span>
          <button onClick={logout} className='py-1.5 px-3 border border-white/20 text-white hover:bg-white/10 text-xs rounded-md cursor-pointer bg-transparent'>Logout
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28">
        <div className="w-full max-w-2xl flex flex-col items-center">
          ...
          <h1 className="text-center text-4xl md:text-6xl font-medium mt-4 max-w-2xl text-white">
            Build your presence on web with <span className="text-red-600">Stackyn AI-Builder</span>
          </h1>
          ...
          {!loadingProjects && projects.length > 0 && (
            <div className='mt-12 w-full'>
              ...
              <div className='flex items-center justify-between pb-3 mb-3 border-b border-white/10'>
                ...
                <span className='text-xs text-xinc-100 font-normal'>
                  {projects.length} {projects.length === 1 ? "project" : "projects"}
                </span>
              </div>

              <div className='space-y-2 max-h-[88vh] overflow-y-auto pr-1'>
                {projects.map((p)=>(
                  ...
                  onClick={()=> navigate(`/builder/${p._id}`)}>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      className='p-1.5 rounded-md text-xinc-200 hover:text-red-400 hover:bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity'>
                        <Trash2Icon size={14}/>
                      </button>
