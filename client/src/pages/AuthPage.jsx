  ...
  return (
    <div className="min-h-screen bg-white flex text-zinc-900 font-sans">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        {error && <div className="mb-6 p-3 border-red-200 bg-red-50 text-red-700 text-xs rounded">{error}</div>}
        ...
        <form className="space-y-6 " onSubmit={handleSubmit}>
          {!isLogin && (
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 transition-colors" placeholder="Prasoon Dwivedi" />
            </div>
          )}
          <div>
              <label className="block text-[11px] font-semibold text-zinc-400 uppercase tracking-widest mb-2" >
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 transition-colors" placeholder="d.prasoon11@gmail.com" />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-zinc-300 hover:text-zinc-600 flex items-center justify-center" >
      ...
      <p className="text-sm text-zinc-400 mt-8 pt-6 border-t border-zinc-100 font-sans">
        {isLogin ?(
          <>
            New to Stackyn AI-Builder?{" "}
            <Link to="/register" className="text-zinc-500 font-medium hover:underline">
              Create an account
            </Link>
          </>
        ) : (
