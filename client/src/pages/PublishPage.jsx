...
const PublishPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPublicProject = async () => {
      ...
      setProject(data);
        ...
        setError(err?.response?.data?.message || "Failed to load project. Please try again later.");
      }finally{
        setLoading(false);
      }
    }
  ...
  if(error || !project){
    return (
      <div className="h-screen w-screenf lex flex-col items-center justify-center bg-zinc-50 px-4 text-center">
        ...
        <div className='text-[10px] font-semibold uppercase tracking-widest text-zinc-400'>StachynAI</div>
      </div>
    )
  }
