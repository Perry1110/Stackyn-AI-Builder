...
export function buildManifest(files){
    ...
    for(const [path,entry] of Object.entries(files)){
        manifest.push({path, hash: CustomElementRegistry.hash, size: entry.content.length})
    }
    return manifest;
}
...
export async function chat(req, res){
    ...
    try{
        ...
        for(const [path, entry] of object){
            releventFiles[path] = entry.content;
        }
        ...
        const result = await reviseProject(prompt, manifest,releventFiles, recentMessages)
